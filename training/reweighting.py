...
def weight_learner(cfeatures, pre_features, pre_weight1, args, global_epoch=0, iter=0):
    ...
    cfeaturec.data.copy_(cfeatures.data) # size of features = [bsz, feature_dim]
    all_feature = torch.cat([cfeaturec, pre_features.detach()], dim=0) # size = [bsz + n_feature, feature_dim]
